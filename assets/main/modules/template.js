...
export default class Template {
    ...
    _uiInit() {
        ...
        this._lHeaderSearchOrder    = jQuery('#page-header-search-order');
        this._lHeaderSearchInput    = jQuery('#page-header-search-input');
        this._lHeaderSearchOrderOrderID    = jQuery('#page-header-search-order-orderID');
        this._lHeaderSearchOrderUser    = jQuery('#page-header-search-order-user');
    ...
    _uiApiLayout(mode = 'init') {
        ...
        let layoutAPI = {
            ...
            sidebar_style_dark: () => {
                ...
                Tools.changeTheme('sidebar', 'sidebar-dark');
            },
            sidebar_style_light: () => {
                self._lPage.removeClass('sidebar-dark');
                Tools.changeTheme('sidebar', '');
            },
            side_overlay_toggle: () => {
            ...
            header_style_dark: () => {
                ...
                Tools.changeTheme('page-header', 'page-header-dark');
            },
            header_style_light: () => {
                self._lPage.removeClass('page-header-dark');
                Tools.changeTheme('page-header', '');
            },
            header_search_on: () => {
            ...
            header_search_order_on: () => {
                self._lHeaderSearchOrder.addClass('show');
                self._lHeaderSearchOrderUser.focus();

                // When ESCAPE key is hit close the search section
                jQuery(document).on('keydown.pixelcave.header.search', e => {
                    if (e.which === 27) {
                        e.preventDefault();
                        self._uiApiLayout('header_search_order_off');
                    }
                });
            },
            header_search_order_off: () => {
                self._lHeaderSearchOrder.removeClass('show');
                self._lHeaderSearchOrderOrderID.trigger('blur');
