...
class AutoRus extends ExcelPrice implements IncomeOrderExcelImpl
{
    public function create(): string
    {
        ...
        $aSheet->getColumnDimension('F')->setWidth(6);
        ...
        foreach ($this->incomeOrder->getIncomes() as $income) {
            ...
            $aSheet->getStyle('E'.$j.'')->applyFromArray($tablePriceFont);
            $aSheet->setCellValue('E'.$j."", str_replace(".", ",", $income->getPriceZak()));

            $aSheet->getStyle('F'.$j.'')->applyFromArray($tablePriceFont);
            $aSheet->setCellValue("F".$j."", $income->getQuantity());
